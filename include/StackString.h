...
{
    for (uint16_t i = 0; i < SS_MAX_LENGTH; i++)
    {
        ss->stack_string[i] = '\0';
    }
    ss->string_length = 0;
 ...
 *          bounds.
 */
static inline uint8_t ss_replace_char_from_index(StackString *ss, uint16_t index, char character)
{
 ...
 */
static inline uint16_t ss_replace_all_chars(StackString *ss, char old_char, char new_char)
{
    uint16_t count = 0;
    for (uint16_t i = 0; i < ss->string_length; i++)
    {
 ...
 */
static inline uint16_t ss_trim_trailing(StackString *ss)
{
    uint16_t count = 0;
 ...
 *          count of whitespace removed.
 */
static inline uint16_t ss_trim_leading(StackString *ss)
{
    uint16_t count = 0;
 ...
 */
static inline uint16_t ss_trim(StackString *ss)
{
    uint16_t count = 0;
...
static inline void ss_strip_all_whitespace(StackString *ss)
{
    for (uint16_t i = 0; i < ss->string_length; i++)
    {
        ...
        {
            for (uint16_t j = i; j < ss->string_length - 1; j++)
            {
 ...
 *          are not.
 */
static inline uint8_t ss_equals(const StackString *ss1, const StackString *ss2)
{
    if (ss1->string_length != ss2->string_length)
        return 0;

    for (uint16_t i = 0; i < ss1->string_length; i++)
    {
 ...
 *          are not.
 */
static inline uint8_t ss_equals_cstr(const StackString *ss, const char *cstr)
{
    uint16_t i = 0;
 ...
 *         modified.
 */
static inline uint8_t ss_truncate(StackString *ss, uint16_t new_length)
{
 ...
 */
static inline uint16_t ss_length(const StackString *ss)
{
 ...
 */
static inline uint16_t ss_to_uppercase(StackString *ss)
{
    uint16_t count = 0;
    for (uint16_t i = 0; i < ss->string_length; i++)
    {
 ...
 * @return The count of characters converted.
 */
static inline uint16_t ss_to_lowercase(StackString *ss)
{
    uint16_t count = 0;
    for (uint16_t i = 0; i < ss->string_length; i++)
    {
